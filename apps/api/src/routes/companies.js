...
router.post('/', auth, async (req, res) => {
  if (req.employee.primaryRole !== 'SUPERADMIN') return res.status(403).json({ error: 'Forbidden' });
...
router.post('/employees', auth, upload.array('documents'), async (req, res) => {
  ...
  const employee = await Employee.create({ name, email, passwordHash, primaryRole: 'EMPLOYEE', subRoles: [role], company: company._id, address, phone, documents });
  res.json({ employee: { id: employee._id, name: employee.name, email: employee.email, subRoles: employee.subRoles } });
});
...
router.get('/employees', auth, async (req, res) => {
  ...
  const company = await Company.findOne({ admin: req.employee.id });
  if (!company) return res.status(400).json({ error: 'Company not found' });
  const employees = await Employee.find({ company: company._id }).select('name email subRoles').lean();
  res.json({ employees: employees.map(u => ({ id: u._id, name: u.name, email: u.email, subRoles: u.subRoles })) });
});
