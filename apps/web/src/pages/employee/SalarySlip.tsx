...
type Field = { key: string; label: string; type: FieldType; required: boolean; category?: FieldCategory };
...
export default function MySalarySlip() {
  ...
  const { earnings, deductions, info, totals } = useMemo(() => {
    const tpl = (template || []).map((f) => ({ ...f, category: (f.category as FieldCategory) || 'info' }));
    const earnings = tpl.filter((f) => f.category === 'earning' && f.type === 'number');
    const deductions = tpl.filter((f) => f.category === 'deduction' && f.type === 'number');
    const info = tpl.filter((f) => f.category !== 'earning' && f.category !== 'deduction');

    const sum = (list: Field[]) => list.reduce((acc, f) => acc + (typeof values[f.key] === 'number' ? Number(values[f.key]) : 0), 0);
    const totalEarnings = sum(earnings);
    const totalDeductions = sum(deductions);
    const netPay = totalEarnings - totalDeductions;
    return { earnings, deductions, info, totals: { totalEarnings, totalDeductions, netPay } };
  }, [template, values]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-1 border border-border rounded-md bg-surface">
          ...
          <div className="divide-y divide-border">
            {earnings.length === 0 && (
              <div className="px-4 py-3 text-sm text-muted">No earning items</div>
            )}
            {earnings.map((f) => (
              <div key={f.key} className="px-4 py-2 flex items-center justify-between">
        ...
        <div className="lg:col-span-1 border border-border rounded-md bg-surface">
          ...
          <div className="divide-y divide-border">
            {deductions.length === 0 && (
              <div className="px-4 py-3 text-sm text-muted">No deduction items</div>
            )}
            {deductions.map((f) => (
              <div key={f.key} className="px-4 py-2 flex items-center justify-between">
                <div className="text-sm text-muted">{f.label}</div>
        ...
        <div className="lg:col-span-1 border border-border rounded-md bg-surface flex flex-col">
          <div className="border-b border-border px-4 py-2 font-semibold">Summary</div>
          <div className="p-4 space-y-2">
            <div className="flex items-center justify-between text-sm">
              <div className="text-muted">Total Earnings</div>
              <div className="font-medium">{formatAmount(totals.totalEarnings)}</div>
            </div>
            <div className="flex items-center justify-between text-sm">
              <div className="text-muted">Total Deductions</div>
              <div className="font-medium">{formatAmount(totals.totalDeductions)}</div>
            </div>
