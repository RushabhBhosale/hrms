...
export default function MonthlyReport() {
  ...
  async function load() {
    ...
    try {
      setLoading(true);
      setErr(null);
      const res = await api.get(`/attendance/monthly/${employeeId}`, { params: { month } });
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-md border border-border bg-surface overflow-hidden">
            <button onClick={() => shiftMonth(-1)} className="px-3 py-2 border-r border-border">Prev</button>
            <input
              type="month"
              value={month}
            ...
            />
            <button onClick={() => shiftMonth(1)} className="px-3 py-2 border-l border-border">Next</button>
          </div>
          <button
            className="rounded-md border border-border px-3 py-2 text-sm"
            onClick={async () => {
              try {
                const name = (employees.find(e => e.id === employeeId)?.name) || employeeId;
                const res = await api.get(`/attendance/monthly/${employeeId}/excel`, {
                  params: { month },
                  responseType: 'blob',
                } as any);
                ...
                a.href = url;
                a.download = `attendance-${name.replace(/\s+/g, '_')}-${month}.xlsx`;
